...
def read_env_d():
    ...
    paths = []
    for name in os.listdir("/etc/env.d"):
        path = os.path.join("/etc/env.d", name)
...
def update_environment():
    env = read_env_d()
    update_file("/etc/profile.env", generate_profile_env(env))
    update_file("/etc/csh.env", generate_profile_env(env, 'setenv %s %s\n'))
    if env.has_key("LDPATH"):
        if update_file("/etc/ld.so.conf", generate_ld_so_conf(env)):
